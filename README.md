# Choreoglyph

Choreoglyph 是一款基于 Swift、SpriteKit 和 GameplayKit 的像素风舞蹈编排解谜游戏。

本仓库是项目作品集，不包含正式产品源码、完整关卡数据或原始美术资源。

## 项目亮点

- 使用纯 Swift 规则引擎承载棋盘、冲突、候选、撤销和通关逻辑；
- 使用 SpriteKit 负责像素风场景、帧动画和交互表现；
- 使用 GameplayKit 状态机管理游戏中的进行、暂停、动画和结算流程；
- 使用配置驱动内容，将角色、动作、关卡与资源引用从场景代码中分离；
- 使用确定性随机种子生成关卡，并通过求解器验证关卡约束和唯一解；
- 通过命令行测试和界面冒烟测试验证核心流程。

## 我的工作

- 产品概念、玩法规则和交互流程设计；
- Swift/SpriteKit 工程搭建与跨 macOS/iOS 适配；
- 规则层与表现层拆分；
- 程序化关卡生成、存档和进度系统设计；
- 主界面帧动画、像素资源加载和交互状态设计；
- 测试方案、问题定位和工程文档整理。

## 文档

- [工程问题与解决思路](docs/engineering-lessons.md)
- [测试与质量说明](docs/testing-and-quality.md)

## 公开范围说明

本仓库只展示项目背景、技术思路和质量实践。正式产品的源代码、完整关卡配置、内部构建日志、原始素材和未发布功能不在公开范围内。

## 素材说明

作品展示所使用的截图和视觉素材仅使用项目原创、用户提供并授权，或项目专用生成素材；如后续添加外部素材，将单独记录来源与授权条件。
